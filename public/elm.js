...
var $author$project$Main$Model = F3(
	function (frameCount, width, height) {
		return {frameCount: frameCount, height: height, width: width};
	});
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$init = function (flags) {
	var w = flags.innerWidth;
	var h = flags.innerHeight;
	var c = 0;
	return _Utils_Tuple2(
		A3($author$project$Main$Model, c, w, h),
...
var $author$project$Main$modByF = F2(
	function (m, f) {
		var intM = $elm$core$Basics$floor(m);
		var intF = $elm$core$Basics$floor(f);
		var modI = A2($elm$core$Basics$modBy, intM, intF);
		var modF = modI;
		var d = f - intF;
		return modF + d;
	});
var $author$project$Main$countToHue = function (c) {
	var m = 200;
	var modC = A2($author$project$Main$modByF, m, c);
	return modC / m;
};
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$core$Basics$degrees(model.frameCount * 0.1))
					])),
				$joakin$elm_canvas$Canvas$Settings$fill(
				A3(
					$avh4$elm_color$Color$hsl,
					$author$project$Main$countToHue(model.frameCount),
					1,
					0.5))
			]),
		_List_fromArray(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
					$elm$core$Basics$floor(model.width),
					$elm$core$Basics$floor(model.height)),
				_List_Nil,
				_List_fromArray(
					[
	...
	A2(
		...
		function (innerWidth) {
			return A2(
				...
				A2($elm$json$Json$Decode$field, 'innerHeight', $elm$json$Json$Decode$float));
		},
		A2($elm$json$Json$Decode$field, 'innerWidth', $elm$json$Json$Decode$float)))(0)}});}(this));